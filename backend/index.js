  ...
  try {
    ...
    if (
      ...
      return res.status(400).json({
        ...
        details: "Please upload a .txt file or .zip file containing .txt files",
        supportedFormats: [".txt", ".zip"],
      });
    }
    ...
    if (!textContent || textContent.trim().length === 0) {
      return res.status(400).json({
        ...
        details:
          "The file is empty or doesn't contain any WhatsApp chat text files",
      });
    }
...
// New function to extract text from ZIP files
  ...
  try {
    ...
    for (const entry of zipEntries) {
      if (
        ...
        console.log(`Found TXT file in ZIP: ${entry.entryName}`);
        txtFilesFound++;
    ...
    console.log(`Successfully extracted ${txtFilesFound} TXT file(s) from ZIP`);
