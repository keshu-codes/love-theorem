...
// ========== FILE UPLOAD AND ANALYSIS - FIXED VERSION WITH ZIP SUPPORT ==========
...
function setupFileUpload() {
  ...
  async function handleFileUpload(file) {
    ...
    // Validate file type - now accepting both .txt and .zip
    const isTxtFile = file.name.toLowerCase().endsWith(".txt");
    const isZipFile = file.name.toLowerCase().endsWith(".zip");

    if (!isTxtFile && !isZipFile) {
      showError(
        "Please upload a .txt file or .zip file (WhatsApp chat export)"
      );
      return;
    }
    ...
    try {
      ...
      formData.append("fileType", isZipFile ? "zip" : "txt");
      ...
      if (!response.ok) {
        ...
        if (errorMessage.includes("No messages could be parsed")) {
          errorMessage =
            "The chat file format is not recognized. Please make sure you exported from WhatsApp correctly.";
        } else if (errorMessage.includes("No valid WhatsApp chat found")) {
          errorMessage =
            "No valid WhatsApp chat found in the ZIP file. Please check the export instructions below.";
        }
  ...
  analysisResult.innerHTML = `
    <div class="bg-red-500/20 border border-red-500/30 rounded-lg p-6">
      <div class="text-center mb-4">
        ...
        <p class="text-red-200 mb-4 whitespace-pre-line">${message}</p>
      </div>
      
      <div class="bg-white/10 rounded-lg p-4 mb-4">
        <h4 class="font-bold mb-3 text-center">📱 Step-by-Step WhatsApp Export Guide:</h4>
        <ol class="text-sm space-y-2 text-left">
          <li class="flex items-start">
            ...
            <span>Open WhatsApp → Open the chat you want to analyze</span>
          </li>
          <li class="flex items-start">
          ...
          <li class="flex items-start">
            ...
            <span>Scroll down and tap <strong>"Export Chat"</strong></span>
          </li>
          <li class="flex items-start">
          ...
          <li class="flex items-start">
            <span class="bg-yellow-500 text-white rounded-full w-6 h-6 flex items-center justify-center text-xs mr-2 flex-shrink-0">5</span>
            <span>Save the file and upload it here (.txt file)</span>
          </li>
        </ol>
        
        <div class="mt-4 p-3 bg-yellow-500/20 rounded-lg border border-yellow-500/30">
          <h5 class="font-bold text-yellow-300 mb-2">💡 Quick Fixes:</h5>
          <ul class="text-xs space-y-1 text-yellow-200">
            <li>• <strong>Use .txt files</strong> instead of .zip for better compatibility</li>
            <li>• Make sure the chat has at least 10+ messages</li>
            <li>• Don't rename the exported file</li>
            <li>• Try a different chat if issues persist</li>
            <li>• Check that WhatsApp export creates a file with chat history</li>
          </ul>
        </div>
      </div>
      ...
      <div class="text-center">
        ...
        <button onclick="navigateToPage('about.html')" class="ml-2 px-4 py-2 bg-blue-600 hover:bg-blue-700 rounded-full text-white text-sm transition transform hover:scale-105">
          Need Help?
        </button>
      </div>
    </div>
  ...
  content.innerHTML = `
    <div class="text-center w-full max-w-lg mx-auto">
      ...
      <div class="upload-area mb-6" id="uploadArea">
        ...
        <p class="text-sm opacity-70 mb-2">Drag & drop your .zip or .txt file or click to browse</p>
        <p class="text-xs opacity-50">File must be exported from WhatsApp as .zip (with media) or .txt (without media)</p>
      ...
      <div class="space-y-6">
        ...
        <div class="bg-white/10 backdrop-blur-lg rounded-xl p-6 border border-white/20">
          ...
          <ul class="text-sm space-y-2 text-left">
            <li>• Upload your WhatsApp chat export (.txt or .zip file)</li>
